...
class MyWidget(core.Widget):
    def __init__(self, name=None, channel=None):
        ...
        self.name = name
        if channel is None:
            self.channel = name  # Default to name if channel is not provided
        else:
...
class MyWidget(core.Widget):
    ...
    def publish(self, message_type, **kwargs):
        # Implement publish method to call the actual publishing mechanism
        # For example, you might call the publish function from the invent module
        invent.publish(message_type, **kwargs)
